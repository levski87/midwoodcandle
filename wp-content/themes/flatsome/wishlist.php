            ...
            if( count( $wishlist ) > 0 ) :
                foreach( $wishlist as $values ) :   
                    ...
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        <td class="product-remove"><div><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remove this product', 'yit' ) ?>"><div class="icon-close"></div></a></td>
